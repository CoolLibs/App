...
ShaderModule::~ShaderModule() {
	if (shader_module != VK_NULL_HANDLE)
		vkDestroyShaderModule(Vulkan::Context::g_Device, shader_module, nullptr);
}
...
ShaderModule ShaderModule::from_spirv_code(ShaderType type, std::string_view spirv_code) {
	...
	if (vkCreateShaderModule(Vulkan::Context::g_Device, &create_info, nullptr, &shader_module.shader_module) != VK_SUCCESS) {
...
GraphicsPipeline::GraphicsPipeline(const std::vector<ShaderModule>& shader_modules) {
	...
	if (vkCreatePipelineLayout(Vulkan::Context::g_Device, &pipeline_layout_info, nullptr, &pipeline_layout) != VK_SUCCESS) {
	...
	if (vkCreateGraphicsPipelines(Vulkan::Context::g_Device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &pipeline) != VK_SUCCESS) {
...
void GraphicsPipeline::create_render_pass() {
	...
	if (vkCreateRenderPass(Vulkan::Context::g_Device, &renderPassInfo, nullptr, &render_pass) != VK_SUCCESS) {
